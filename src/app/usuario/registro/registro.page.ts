...
export class RegistroPage implements OnInit {
  ...
  constructor(
    ...
    this.registro = this.fb.group({
      ...
      Validators.maxLength(10),this.AEvalidarCedula]],
      ...
      password: [
        ...
        [
          ...
          Validators.pattern('^(?=.*[a-z]{2})(?=.*[A-Z]{2})(?=.*[0-9])(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&].{8,}$'),
        ],
      ],
      confirmPassword: [
        '',
  ...
  AEvalidarCedula(control: AbstractControl): ValidationErrors | null {
    ...
    if (cedula.length === 10) {
      ...
      if (digito_region >= 1 && digito_region <= 24) {
        ...
        const pares = parseFloat(cedula.substring(1, 2)) + parseFloat(cedula.substring(3, 4)) +
                    parseFloat(cedula.substring(5, 6)) + parseFloat(cedula.substring(7, 8));
        let numero1 = parseFloat(cedula.substring(0, 1));
        numero1 = (numero1 * 2);
        if (numero1 > 9) { numero1 = (numero1 - 9); }
        let numero3 = parseFloat(cedula.substring(2, 3));
        numero3 = (numero3 * 2);
        if (numero3 > 9) { numero3 = (numero3 - 9); }
        let numero5 = parseFloat(cedula.substring(4, 5));
        numero5 = (numero5 * 2);
        if (numero5 > 9) { numero5 = (numero5 - 9); }
        let numero7 = parseFloat(cedula.substring(6, 7));
        numero7 = (numero7 * 2);
        if (numero7 > 9) { numero7 = (numero7 - 9); }
        let numero9 = parseFloat(cedula.substring(8, 9));
        numero9 = (numero9 * 2);
        ...
        const decena = (parseFloat(primer_digito_suma) + 1) * 10;
        let digito_validador = decena - suma_total;
        if (digito_validador === 10) { digito_validador = 0; }
        if (digito_validador === ultimo_digito) { return null; }
      }
    }
